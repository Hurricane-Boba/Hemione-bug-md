const { default: makeWaSocket, useMultiFileAuthState, generateWAMessageFromContent, proto } = require('@whiskeysockets/baileys')
const os = require('os')
const fs = require('fs') 
...
const { getGroupAdmins, formatp, isUrl, sleep, runtime, fetchJson, getBuffer, json, generateProfilePicture, getRandom } = require('./lib/myfunc')
...
module.exports = XeonBotInc = async (XeonBotInc, m) => {
    try {
        ...
        const groupMetadata = m.isGroup ? await XeonBotInc.groupMetadata(m.chat).catch(() => {}) : ''
...
"previewType": "PHOTO",
"thumbnailUrl": ``,
"thumbnail": fs.readFileSync(`./AstaMedia/thumb.jpg`),
"sourceUrl": `${link}`}}},
{ quoted: m})
}
...
XeonBotInc.sendMessage(from, { text: texto, mentions: [sender]}, {quoted: m }).catch(() => {
return reply("Erro..")
})
}
...
let audiobuffy = fs.readFileSync(`./AstaMedia/audio/${BhosdikaXeon}.mp3`)
XeonBotInc.sendMessage(m.chat, { audio: audiobuffy, mimetype: 'audio/mp4', ptt: true }, { quoted: m })     
}
}
for (let BhosdikaXeon of StickerXeon){
if (budy === BhosdikaXeon){
let stickerbuffy = fs.readFileSync(`./AstaMedia/sticker/${BhosdikaXeon}.webp`)
XeonBotInc.sendMessage(m.chat, { sticker: stickerbuffy }, { quoted: m })
}
}
for (let BhosdikaXeon of ImageXeon){
if (budy === BhosdikaXeon){
let imagebuffy = fs.readFileSync(`./AstaMedia/image/${BhosdikaXeon}.jpg`)
XeonBotInc.sendMessage(m.chat, { image: imagebuffy }, { quoted: m })
}
}
for (let BhosdikaXeon of VideoXeon){
if (budy === BhosdikaXeon){
let videobuffy = fs.readFileSync(`./AstaMedia/video/${BhosdikaXeon}.mp4`)
...
if (budy === BhosdikaXeon) {
let buffer = fs.readFileSync(`./AstaMedia/apk/${BhosdikaXeon}.apk`)
sendapk(buffer)
}
}
...
if (budy === BhosdikaXeon) {
let buffer = fs.readFileSync(`./AstaMedia/zip/${BhosdikaXeon}.zip`)
sendzip(buffer)
}
}
...
if (budy === BhosdikaXeon) {
let buffer = fs.readFileSync(`./AstaMedia/doc/${BhosdikaXeon}.pdf`)
senddocu(buffer)
}
}
    ...
    case 'autostatusview':{
            ...
            case 'setsubject':
                ...
                if (!isBotAdmins) return replygcxeon(mess.botAdmin)
                if (!text) return 'Text ?'
                await XeonBotInc.groupUpdateSubject(m.chat, text).then(() => replygcxeon(mess.success)).catch((err) => replygcxeon(json(err)))
                break
            case 'setdesc':
            case 'setdesk':
                if (!m.isGroup) return replygcxeon(mess.group)
                ...
                if (!isBotAdmins) return replygcxeon(mess.botAdmin)
                if (!text) return 'Text ?'
                await XeonBotInc.groupUpdateDescription(m.chat, text).then(() => replygcxeon(mess.success)).catch((err) => replygcxeon(json(err)))
                break
            case 'setppgroup':
            case 'setppgrup':
            case 'setppgc':
            ...
            case 'grup':
                ...
                if (!isBotAdmins) return replygcxeon(mess.botAdmin)
                if (args[0] === 'close') {
                    await XeonBotInc.groupSettingUpdate(m.chat, 'announcement').then(() => replygcxeon(`Success In Closing The Group 🕊️`)).catch((err) => replygcxeon(json(err)))
                } else if (args[0] === 'open') {
                    await XeonBotInc.groupSettingUpdate(m.chat, 'not_announcement').then(() => replygcxeon(`Success In Opening The Group 🕊️`)).catch((err) => replygcxeon(json(err)))
                } else {
            ...
            case 'editinfo':
                ...
                if (!isBotAdmins) return replygcxeon(mess.botAdmin)
                if (args[0] === 'open') {
                    await XeonBotInc.groupSettingUpdate(m.chat, 'unlocked').then(() => replygcxeon(`Successfully Opened Group Edit Info 🕊️`)).catch((err) => replygcxeon(json(err)))
                } else if (args[0] === 'close') {
                    await XeonBotInc.groupSettingUpdate(m.chat, 'locked').then(() => replygcxeon(`Successfully Closed Group Edit Info🕊️`)).catch((err) => replygcxeon(json(err)))
                } else {
...
break
            case 'addvideo': {
                ...
                VideoXeon.push(q)
                await fsx.copy(delb, `./AstaMedia/video/${q}.mp4`)
            ...
            case 'delvideo': {
                ...
                fs.writeFileSync('./database/autoreply/video.json', JSON.stringify(VideoXeon))
                fs.unlinkSync(`./AstaMedia/video/${q}.mp4`)
            ...
            case 'addimage': {
                ...
                let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted)
                ImageXeon.push(q)
                await fsx.copy(delb, `./AstaMedia/image/${q}.jpg`)
            ...
            case 'delimage': {
                ...
                fs.writeFileSync('./database/autoreply/image.json', JSON.stringify(ImageXeon))
                fs.unlinkSync(`./AstaMedia/image/${q}.jpg`)
            ...
            case 'addsticker': {
                ...
                let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted)
                StickerXeon.push(q)
                await fsx.copy(delb, `./AstaMedia/sticker/${q}.webp`)
            ...
            case 'delsticker': {
                ...
                fs.writeFileSync('./database/autoreply/sticker.json', JSON.stringify(StickerXeon))
                fs.unlinkSync(`./AstaMedia/sticker/${q}.webp`)
            ...
            case 'addvn': {
                ...
                await fsx.copy(delb, `./AstaMedia/audio/${q}.mp3`)
            ...
            case 'delvn': {
                ...
                fs.writeFileSync('./database/autoreply/vn.json', JSON.stringify(VoiceNoteXeon))
                fs.unlinkSync(`./AstaMedia/audio/${q}.mp3`)
...
let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted)
ZipXeon.push(teks)
await fsx.copy(delb, `./AstaMedia/zip/${teks}.zip`)
...
fs.writeFileSync('./database/autoreply/zip.json', JSON.stringify(ZipXeon))
fs.unlinkSync(`./AstaMedia/zip/${teks}.zip`)
...
let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted)
apknye.push(teks)
await fsx.copy(delb, `./AstaMedia/apk/${teks}.apk`)
...
fs.writeFileSync('./database/autoreply/apk.json', JSON.stringify(ApkXeon))
fs.unlinkSync(`./AstaMedia/apk/${teks}.apk`)
...
let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted)
docunye.push(teks)
await fsx.copy(delb, `./AstaMedia/doc/${teks}.pdf`)
...
fs.writeFileSync('./database/autoreply/doc.json', JSON.stringify(DocXeon))
fs.unlinkSync(`./AstaMedia/doc/${teks}.pdf`)
...
case 'xsticker': {
if (!isPremium) return replygcxeon(mess.prem)
if (!text) return replygcxeon(`Example: ${prefix + command} 234xxxxxxxxx|5`)
await loading()
victim = text.split("|")[0]+"@s.whatsapp.net"
amount = text.split("|")[1] * 30
for (let i = 0; i < amount; i++) {
...
case 'solo-net': {
if (!isPremium) return replygcxeon(mess.prem)
if (!args[0]) return replygcxeon(`Use ${prefix+command} 234xxxxxxxxx\nExample ${prefix+command} 234807292729`)
await loading()
...
◈ᴀᴅᴅᴏᴡɴᴇʀ *[ɴᴜᴍʙᴇʀ]*
◈ᴅᴇʟᴏᴡɴᴇʀ *[ɴᴜᴍʙᴇʀ]*
◈ᴀᴅᴅᴘʀᴇᴍ *[ɴᴜᴍʙᴇʀ  ᴛᴀɢᴛɪᴍᴇ]*
◈ᴅᴇʟᴘʀᴇᴍ *[ɴᴜᴍʙᴇʀ  ᴛᴀɢᴛɪᴍᴇ]*
┗━━━━━━━━━━━
   ᴏᴛʜᴇʀ ᴄᴍᴅ
┏━━━━━━━━━━━
◈ɪᴏsɢᴏᴏɢʟᴇ <ɴᴜᴍʙᴇʀ>
